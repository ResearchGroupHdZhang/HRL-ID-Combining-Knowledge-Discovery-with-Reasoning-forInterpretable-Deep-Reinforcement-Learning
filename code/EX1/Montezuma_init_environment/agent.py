...
block_encoding = {"Man":0, "Ladder":1, "Key":2,"MiddleLadder":3,"LeftLadder":4,"RightLadder":5,"Skull":6}
block_encoding_2 = {0 : "Man", 1 : "Ladder", 2 : "Key",3:"MiddleLadder",4:"LeftLadder",5:"RightLadder",6:"Skull"}
idx_to_subgoal = {0:-1,1:6,2:2,3:5,4:3,5:0,6:1}
idx2block = {}
...
class LinearSchedule(object):
    ...
    def value(self, t, performance):
        fraction = min(float(t) / self.schedule_timesteps, 1.0)
        if performance < 0.1:
            return self.initial_p + fraction * (self.final_p - self.initial_p) * (performance + 0.9)
        elif performance >=0.1 and performance <0.2:
            return self.initial_p + fraction * (self.final_p - self.initial_p) * (performance + 0.8)
        elif performance >=0.2 and performance <0.3:
            return self.initial_p + fraction * (self.final_p - self.initial_p) * (performance + 0.7)
        elif performance >= 0.3 and performance < 0.4:
            return self.initial_p + fraction * (self.final_p - self.initial_p) * (performance + 0.6)
        elif performance >= 0.4 and performance < 0.5:
            return self.initial_p + fraction * (self.final_p - self.initial_p) * (performance + 0.5)
        elif performance >= 0.5 and performance < 0.6:
            return self.initial_p + fraction * (self.final_p - self.initial_p) * (performance + 0.4)
        elif performance >= 0.6 and performance < 0.7:
            return self.initial_p + fraction * (self.final_p - self.initial_p) * (performance + 0.3)
        elif performance >= 0.7 and performance < 0.8:
...
rules_set_allObject_inAgent = [[] for _ in range(Num_subgoal)]
flag_in = 0
...
class SampleEnv():
    ...
    def run_episode(self,input_data):
        ...
        if len(succcess_rate_Copy) == 0:
            ...
            for number in range(Num_subgoal):
                if np.abs(
                        success_rate[number] - succcess_rate_Copy[number]) >= 0.1:
                    succcess_rate_Copy = success_rate
                    self.test_for_update_rules()
                    break
        # rules_set_allGoal = input_data[1]
        ...
        while not self.env.isTerminal() and episodeSteps <= maxStepsPerEpisode and episodeGoalCount <= 7:
            ...
            # if random_num < max_rate:
            # print("change goal to:",final_rule_goal)
            # if subgoal == -1:
            #     print("change failed")
    ...
    def test_for_update_rules(self):
        ...
        while not self.env.isTerminal() and episodeSteps <= maxStepsPerEpisode:
            ...
            meta_batch = Batch(obs=[meta_obs])
            batch = self.metaAgent(meta_batch,eps = 0)
            with torch.no_grad():
...
class SubprocSampleEnv():
    ...
    def run_episode(self,option_performance,rules_set_allGoal):
        #input_Data = [option_performance,rules_set_allGoal]
        input_Data = [option_performance,rules_set_allGoal]
