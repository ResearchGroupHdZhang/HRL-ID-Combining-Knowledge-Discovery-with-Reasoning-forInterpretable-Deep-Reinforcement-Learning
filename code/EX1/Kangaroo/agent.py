...
class SampleEnv():
    ...
    def run_episode(self,input_data):
        ...
        if len(succcess_rate_Copy) == 0:
            ...
            for number in range(Num_subgoal):
                if np.abs(
                        success_rate[number] - succcess_rate_Copy[number]) >= 0.05:
                    succcess_rate_Copy = success_rate
                    self.test_for_update_rules()
                    break
        ...
        while not self.env.isTerminal() and episodeSteps <= maxStepsPerEpisode and episodeGoalCount <= 4:
            ...
            meta_obs_memory.append(meta_obs)
            ...
            if subgoal != -1:
                ...
                while not self.env.isTerminal() and not reach_goal == goal and episodeSteps <= maxStepsPerEpisode:
                    ...
                    #     self.env.ale.saveScreenPNG('/root/hrlid-master/Game/image/'+str(data)+str(tag)+'.png')
                    #     tag += 1
                ...
                if args.model != 'hdqn' and (done or episodeSteps > maxStepsPerEpisode):
                    meta_obs_next = np.ones_like(meta_obs) * subgoal / 4
                    done = True
                else:
                    if reach_goal == goal and goal == 3:
                ...
                if compute_goal != act:
                    print("can't match!!!")
                    if args.model != 'hdqn':
                        meta_obs_next = np.ones_like(meta_obs_memory[episodeGoalCount-1]) * (-1) / 4
                    else:
                        meta_obs_next = obs_fn(self.env)
                    self.meta_experience.append([meta_obs_memory[episodeGoalCount-1], compute_goal, -150 / 20, True, meta_obs_next])
                    # self.meta_experience.append([meta_obs_memory[episodeGoalCount-1], compute_goal, -150 / 20, True, meta_obs_next])
                    # self.meta_experience.append([meta_obs_memory[episodeGoalCount-1], compute_goal, -150 / 20, True, meta_obs_next])
                else:
                    print("match~")
    ...
    def setset(self):
        return self.env.ale.saveScreenPNG('/root/hrlid-master/Game/image/0.png')
    def test_for_update_rules(self):
        ...
        while not self.env.isTerminal() and episodeSteps <= maxStepsPerEpisode:
            ...
            flag = 0
            path = '/root/hrlid-master/Game/image/'
            # path = '/home/zengxy/hrlid-master/Game/image/'
    ...
    def test(self):
        ...
        while not self.env.isTerminal() and episodeSteps <= maxStepsPerEpisode:
            ...
            subgoal = self.env.select_goal_(goal, reach_goal)
            print("reach_goal:",reach_goal,"  goal:",goal,"  subgoal:",subgoal)
            flag = 0
            if subgoal != -1:
