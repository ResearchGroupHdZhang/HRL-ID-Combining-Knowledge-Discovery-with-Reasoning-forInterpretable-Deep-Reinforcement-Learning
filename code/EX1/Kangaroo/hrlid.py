...
agent_exploration_steps[1] = 40000
agent_exploration_steps[2] = 80000
agent_exploration_steps[3] = 50000
# agent_exploration_steps[4] = 120000
# agent_exploration_steps[5] = 60000
# agent_exploration_steps[6] = 60000

save_learned = [False for _ in range(5)]
...
agent_list, metaAgent = create_pair_model(agent_eps=agent_eps,
                                          ...
                                          device='cuda:' + str(
                                              args.gpu) if args.gpu >= 0 and torch.cuda.is_available() else 'cpu')
...
# option_learned[0] = True
# option_learned[1] = True
# option_learned[2] = True
...
def load_weightsss():
    for i in range(2):
        agent_list[i].load_weights('./Model/hrlid/', str(i), True)
        agent_list[i].eval()
        agent_list[i].set_eps(0)
    metaAgent.load_weights('./Model/hrlid/', 'meta', True)
# env.setset()
...
if args.mode == 'train':
    ...
    with tqdm.tqdm(total=args.episode_limit, desc='Train', **tqdm_config) as t:
        ...
        while episodeCount <= t.total:
            # env.ale.saveScreenPNG('/root/hrlid/Game/image/0.png')
            env.set_option_learned(option_learned)

            sample = env.run_episode(option_performance)
            ...
            for idx in range(num):  # every env
                ...
                for key in agent_experience.keys():
                    # print("agent_experience:",agent_experience)
            ...
            # anneal subgoal eps
            for goal in goal_to_train:
                agent_list[goal].anneal_eps(option_t[goal], option_learned[goal], option_learned[goal])
