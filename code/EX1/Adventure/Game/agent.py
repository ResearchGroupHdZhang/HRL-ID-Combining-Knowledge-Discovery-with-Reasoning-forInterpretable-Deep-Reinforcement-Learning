...
actionMap = [2, 3, 4, 5]
goal_to_train = [i for i in range(2)]
...
class Model(nn.Module):
    ...
    def __init__(self, input_channel=4, output_dim=4):
        super().__init__()
...
class SampleEnv():
    ...
    def run_episode(self, input_data):
        ...
        while not self.env.isTerminal() and episodeSteps <= maxStepsPerEpisode and episodeGoalCount <= 2:
            ...
            reach_goal = current_reached_goal
            print("reach goal:", reach_goal)
            print("loc:", self.env.getLoc(self.env.ale.getScreenRGB(), 'Man'))
            print("goal:", goal)
            ...
            if subgoal != -1:
                ...
                while not self.env.isTerminal() and not reach_goal == goal and episodeSteps <= maxStepsPerEpisode:
                    ...
                    # print("reach_goal1111111:", reach_goal)
                    if reach_goal == 1:
                        self.env.isKeyExist = False
                        print("reach_goal======Key!!!")
                        tmp_rew = 100
                        current_reached_goal = reach_goal

                    elif reach_goal == 2:
                        print("reach_goal======Door!!!")
                        tmp_rew = 300
                        current_reached_goal = reach_goal
                    ...
                    # intrinsicRewards = self.agent_list[subgoal].criticize(reach_goal == goal, done, self.env.distanceReward(lastgoal=current_reached_goal, goal=goal), False)
                    intrinsicRewards = self.agent_list[subgoal].criticize(reach_goal == goal, done, 0, True)

                    self.episodeGoal_experience[subgoal].append([obs, act, intrinsicRewards + tmp_rew, done, obs_next])
                ...
                if args.model != 'hdqn':
                    meta_obs_next = np.ones_like(meta_obs) * subgoal / 3
                else:
                    meta_obs_next = obs_fn(self.env)
...
def create_pair_model_pg(agent_eps=None,
    ...
    return agent_list, metaAgent
block_encoding = {"Man":0, "Key":1,"Door":2}
block_encoding_2 = {0 : "Man", 1 : "Key", 2 : "Door"}
idx2block = {}
