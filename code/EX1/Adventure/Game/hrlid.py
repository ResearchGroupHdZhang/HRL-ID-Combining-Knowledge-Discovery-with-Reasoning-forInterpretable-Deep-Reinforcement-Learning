...
agent_exploration_steps[0] = 300000
agent_exploration_steps[1] = 300000
...
save_learned = [False for _ in range(3)]
...
if args.mode == 'train':
    ...
    for i in range(2):
        agent_list[i].load_weights('./logdir/hrlid', str(i), True)
        t = agent_list[i].eval()
        agent_list[i].set_eps(0)

    metaAgent.load_weights('./logdir/hrlid', 'meta', True)
    metaAgent.eval()
    metaAgent.set_eps(0)
    sync_process_weights()
    test_external_rew = env.test()
