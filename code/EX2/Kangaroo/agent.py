...
subgoal_togoal = {"LowLadder":0,"Collection1":1,"MiddleLadder":2,"Collection3":3}
subgoal_togoal_2 = {0:"LowLadder",1:"Collection1",2:"MiddleLadder",3:"Collection3"}
block_encoding = {"Man":0, "LowLadder":1, "MiddleLadder":2, "Collection3":3, "Collection1":4, "Collection2":5, "HighLadder":6}
block_encoding_2 = {0 : "Man", 1 : "LowLadder", 2 : "MiddleLadder", 3 : "Collection3", 4 : "Collection1", 5 : "Collection2", 6 : "HighLadder"}
rule_mapping = {
    "Collection" : ["Collection1","Collection3"],
    "CommenObject" : ["LowLadder","MiddleLadder","HighLadder"]
}
idx2block = {}
...
class LinearSchedule(object):
    ...
    def value(self, t, performance):
        return max(0.01,0.995*performance)
    def value_(self, t, performance):
...
actionMap = [2, 3, 4]
goal_to_train = [i for i in range(4)]
    ...
    def anneal_eps(self, step_count, option_learned=False, option_performance=0):
        if option_learned:
            ...
            if step_count > self.random_play_steps:
                self.set_eps(self.exploration.value_(step_count - self.random_play_steps,option_performance))

    def anneal_eps_(self, step_count, option_learned=False, eps=0):
        if option_learned:
            self.set_eps(0)
        else:
            if step_count > self.random_play_steps:
                self.set_eps(self.exploration.value(step_count - self.random_play_steps,eps))
...
class Model(nn.Module):
    ...
    def __init__(self, input_channel=4, output_dim=3):
        super().__init__()
...
model_fns = {'hdqn': lambda: Model(output_dim=7),
             ...
             'hrlid': lambda: MetaModel(predicate_num=args.predicate_num,
...
class SampleEnv():
    ...
    def run_episode(self,input_data):
        ...
        may_use_rules_goal_list = [i for i in rule_mapping[may_use_rules_goal]] #Collection1 Collection2 Collection3
        ...
        for raw_rule in raw_add_rules:
            for may_use_goal in may_use_rules_goal_list:
                processed_use_rule = [may_use_goal if str == may_use_rules_goal else str for str in raw_rule] #["0","0","Man", "Collection123", "ActorWithOutObject"]
        ...
        if len(succcess_rate_Copy) == 0:
            ...
            for may_goal_rule in may_use_rules_goal_list:
                if success_rate[subgoal_togoal[may_goal_rule]] > 0:
                    rules_set_allObject_inAgent[subgoal_togoal[may_goal_rule]] = pre_rules_set_allObject_inAgent[subgoal_togoal[may_goal_rule]]
        ...
        lastgoal = -1
        reach_goal = 0
        while not self.env.isTerminal() and episodeSteps <= maxStepsPerEpisode and episodeGoalCount <= 4:
            ...
            subgoal = self.env.select_goal_(goal, reach_goal)
            episodeGoalCount += 1
            ...
            for may_goal in may_goal_list:
                ...
                tmp_subgoal = self.env.select_goal_(may_goal, reach_goal)
                if tmp_subgoal == -1:
                    continue
            ...
            if subgoal != -1:
                ...
                while not self.env.isTerminal() and not reach_goal == goal and episodeSteps <= maxStepsPerEpisode:
                    ...
                    self.steps += 1
                    # if data==1 and goal==3:
                    #     self.env.ale.saveScreenPNG('/root/xxx-master/Game/image/'+str(data)+str(tag)+'.png')
                    #     tag += 1
                    ...
                    act = result.act[0]
                    # if data == 1:
                    #     print("choose action:", act,"    actionMap[act]:",actionMap[act])
                    ...
                    distanceRew = self.env.distanceReward(lastgoal,goal)
                    intrinsicRewards = self.agent_list[subgoal].criticize(reach_goal == goal, done, distanceRew, True)
                ...
                if episodeSteps > maxStepsPerEpisode or self.env.isTerminal():
                    ...
                    print("reach_goal == goal:",reach_goal)
    ...
    def setset(self):
        return self.env.ale.saveScreenPNG('/root/xxx-master/Game/image/0.png')
    def test_for_update_rules(self):
        ...
        while not self.env.isTerminal() and episodeSteps <= maxStepsPerEpisode:
            ...
            meta_batch = Batch(obs=[meta_obs])
            batch = self.metaAgent(meta_batch)
            with torch.no_grad():
            ...
            # path = '/home/zengxy/xxx-master/Game/image/'
            ...
            if subgoal == -1:
                break
            atoms = compute_atoms(goal)
            rules_for_this_goal, possible_path = visualize_logical_path(meta_obs, batch.path_attn,
                                                                        batch.pre_attn_list,
                                                                        atoms)
            rules_set_allObject_inAgent[goal] = rules_for_this_goal
...
class SubprocSampleEnv():
    ...
    def setset(self):
        ...
        for i in id:
            self.parent_remote[i].send(['setset', None])
        sample = [self.parent_remote[i].recv() for i in id]
        return sample

    def run_episode＿(self):

        id = range(self.env_num)
        for i in id:
            if i == 0:
                self.parent_remote[i].send(['run_episode', 1])
            else:
